...
## In Progress
- **2026-09-01 — GROK — IN PROGRESS — Web Sentry SDK (`@sentry/browser`, DealDex Vite helper pattern) + `logo.match` metric (board 86902ab6, worktree `~/apps/contactlogo-grok-sentry-adopt` @ `grok/sentry-fleet-adoption`).**  Replay 100% error / 10% session, feedback widget on, sendDefaultPii false.  Android Sentry is iOS-only until Android ships.
